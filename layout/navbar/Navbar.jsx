...
const Navbar = () => {
  ...
  return (
    <Fragment>
      {/* Desktop Header */}
      <div
        className='w-full h-[70px] px-8 bg-[rgba(234,230,230,0.8)] dark:bg-[rgba(0,0,0,0.8)] backdrop-filter backdrop-blur-lg hidden md:flex justify-between items-center gap-4 
        shadow-sm shadow-gray-300 dark:shadow-gray-800 fixed z-10 transition-all duration-500 rounded-full'
        style={{ top: top }}
      >
        {/* Name Logo */}
        <p className='text-gray-400 flex'>
          <span className='text-2xl font-bold text-red-900'>LA</span> 
        </p>
        <div className='h-full flex gap-4'>
          {/* Navbar Links */}
          {NavbarMenu.map((navbar) => (
            <Link
              className={"text-blue-800 dark:text-[#07d0e5] font-bold text-lg"}
              href={navbar.link}
              key={navbar.name}
            >
