  ...
  return (
    <Fragment>
      ...
      >
        ...
        >
          <button
            className={`w-[120px] md:w-[150px] p-2 font-bold ${
              section === 'Advance' ? 'bg-blue-400' : null
            } transition-all`}
            onClick={(e) => {
          ...
          <button
            className={`w-[100px] md:w-[150px] p-2 font-bold ${
              section === 'Familiar' ? 'bg-blue-400' : null
            } transition-all`}
            onClick={(e) => {
              setSection(e.target.innerText)
              if (section !== e.target.innerText)
        ...
        <div
          className="pop-down-child flex min-h-[450px] py-[30px] px-[20px] md:px-[100px] flex-wrap justify-center items-center gap-3"
          ref={techBoxesRef}
        >
          {sectionData.map((tech) => (
            <div
              className="transition-all duration-700 px-2 h-fit py-3 md:py-5 w-[100px] md:w-[110px] 
               rounded-lg flex flex-col gap-2 items-center border bg-[#faf7f7] dark:bg-[#1a1919]
                hover:bg-[#c7c6c6] dark:hover:bg-[#333333] hover:scale-110"
              key={tech.name}
            >
              <p>{tech.icon}</p>
              <p>{tech.name}</p>
